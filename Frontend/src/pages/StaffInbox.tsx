...
const StaffInbox = () => {
  ...
  // 1. Fetch Client Details (to get their real Company Name)
  useEffect(() => {
    if (!clientId) {
    ...
    const fetchClientDetails = async () => {
        try {
            ...
            setClientName(res.data?.companyName || `Client ID: ${clientId.substring(0,6)}`);
        } catch (err) {
  ...
  // 2. Fetch Messages, Mark as Read & Setup Polling
  useEffect(() => {
    if (!clientId) return;
    ...
    const fetchMessages = async () => {
      try {
        ...
        // Tell the database the staff has read these messages to clear the red dot!
        await api.put(`/messages/admin/mark-read/${clientId}`);
      } catch (err) {
    ...
    // Auto-refresh every 3 seconds to check for new client replies
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      // Optimistic update for a snappy UI feel
      const optimisticMessage = { senderRole: 'staff', text: message, _id: Date.now() };
      setMessages((prev) => [...prev, optimisticMessage]);
      setMessage(""); 

      // Save to database
      await api.post(`/messages/${clientId}`, {
        senderRole: 'staff', // We are the staff!
            ...
            <div className="flex-1 bg-gray-50 p-6 overflow-y-auto space-y-4 flex flex-col">
                {clientId ? (
                     <>
                        ...
                        {/* Map out the real messages */}
                        {messages.map((msg) => {
                            const isMe = msg.senderRole === 'staff';
                            return (
                                <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[75%] px-5 py-3 rounded-2xl text-lg shadow-sm ${
                                        isMe 
            ...
            <div className="p-4 border-t border-gray-200 bg-white">
                <form onSubmit={handleSend} className="relative">
                    ...
                    <button 
                        ...
                        className={`absolute right-4 top-1/2 -translate-y-1/2 transition p-2 disabled:opacity-50 ${!clientId ? 'text-gray-300' : 'text-[#FE5C00] hover:text-orange-700'}`}
                    >
                        <svg
